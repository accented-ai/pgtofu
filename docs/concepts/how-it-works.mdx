...
When statically identifiable output columns change names at the same ordinal,
pgtofu emits explicit `ALTER VIEW ... RENAME COLUMN` statements before replacing
the definition. Changes whose prior definitions depend on other changed objects
remain in one migration batch, with tables and views restored before dependent
indexes, functions, and triggers in down migrations.
