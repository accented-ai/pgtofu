...
func TestNormalizeViewDefinitionHandlesPostgresJoinParenthesesAndIsDistinctFrom(t *testing.T) {
	t.Parallel()
...
WHERE
    ...
    AND collection.category IS DISTINCT FROM 'archived'
    AND (
...
`
 ...
 SELECT link.id,
  ...
  WHERE ((item.visibility = 'public'::text)
    AND (record.status = 'published'::text)
    AND (collection.category IS DISTINCT FROM 'internal'::text)
    AND (collection.category IS DISTINCT FROM 'archived'::text)
